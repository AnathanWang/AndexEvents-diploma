...
**Обновлено:** 16 марта 2026 (matching + stability updates)  
...
## ✅ **Обновления от 16 марта 2026 (сделано)**

- ✅ Добавлен backend endpoint `GET /api/matches/incoming-likes` (кто лайкнул меня, но я еще не ответил)
- ✅ Добавлен UI-фильтр `Меня лайкнули` в профиле
- ✅ Добавлен сервисный метод `getIncomingLikes()` во Flutter
- ✅ Исправлена SQL-ошибка `500` при `POST /api/matches/like|dislike` (quoted identifiers для `"userAAction"/"userBAction"`)
- ✅ Исправлено залипание карточки после свайпа (корректный `setState` при переходе на следующую карточку)
- ✅ Добавлен ручной сброс локального seen-кэша и перезагрузка подборки (`Обновить подборку`)
- ✅ Добавлены SQL-скрипты для локального тестирования fake-профилей и очистки
...
#### ✅ 2. **Валидация email адреса при регистрации** — ВЫПОЛНЕНО (12 марта 2026)
...
#### ✅ 3. **Восстановление пароля** — ВЫПОЛНЕНО (12 марта 2026)
...
#### 4. **Deep Links и Universal Links**
