...
export const createOrUpdateMatch = async (userAId, userBId, action) => {
    logger.info(`[createOrUpdateMatch] Creating/updating match between ${userAId} and ${userBId} with action ${action}`);
    try {
        ...
        if (!match) {
            ...
            if (!match) {
                // Создаём новый матч
                logger.info(`[createOrUpdateMatch] Creating new match between ${userAId} and ${userBId}`);
                ...
                logger.info(`[createOrUpdateMatch] Updating existing reverse match between ${userBId} and ${userAId}`);
            ...
            logger.info(`[createOrUpdateMatch] Updating existing match between ${userAId} and ${userBId}`);
            // Проверяем, есть ли взаимный лайк
            const isMutual = _isMutualLike(match.userBAction, action);
            match = await prisma.match.update({
                where: {
        ...
        logger.error(`[createOrUpdateMatch] Error creating/updating match between ${userAId} and ${userBId}:`, error);
        throw error;
    }
};
/**
 ...
 */
function _isMutualLike(action1, action2) {
    if (!action1) {
        return false;
    }
 ...
 */
export const getUserActions = async (userId, limit = 50) => {
