...
class EventController {
  ...
  async getAllEvents(req: Request, res: Response) {
    try {
      const { latitude, longitude, maxDistance, category, page, limit } =
        req.query;
      ...
      if (latitude && longitude) {
        ...
        const distance = maxDistance
          ? Number.parseInt(maxDistance as string, 10)
          : 50000;
  ...
  async participateInEvent(req: AuthRequest, res: Response) {
    try {
      ...
      const participation = await eventService.participateInEvent(
        eventId,
        userId,
        status,
      );
